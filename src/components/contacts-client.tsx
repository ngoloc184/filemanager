...
export default function ContactsClient({ initialUsers }: { initialUsers: RelatedUser[] }) {
  ...
  return <div className="max-w-2xl space-y-6">
    ...
    <Card><CardHeader><CardTitle>Add a related user</CardTitle><CardDescription>Enter the email address they used to register.</CardDescription></CardHeader><CardContent>
      <form onSubmit={addUser} className="flex gap-2"><Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="user@example.com" required /><Button type="submit" disabled={loading}><UserPlus className="h-4 w-4 mr-2" />{loading ? "Adding..." : "Add"}</Button></form>
    </CardContent></Card>
